...
<nav class={cn('grid gap-3', $$restProps.class)}>
	...
	<a
		...
		class={cn(
			...
			$page.url.pathname === '/dev/dashboard' ? 'bg-muted' : 'text-muted-foreground'
		)}
	...
	<a
		href="/dev/transactions"
		class={cn(
			'mx-[-0.65rem] flex items-center  px-3 hover:text-foreground',
			isSheetMode ? 'gap-4 rounded-xl py-2 text-lg font-medium' : 'gap-2 rounded-md py-1',
			$page.url.pathname === '/dev/transactions' ? 'bg-muted' : 'text-muted-foreground'
		)}
		on:click={onItemClicked}
	>
	...
	<a
		...
		class={cn(
			'mx-[-0.65rem] flex items-center  px-3 hover:text-foreground',
			isSheetMode ? 'gap-4 rounded-xl py-2 text-lg font-medium' : 'gap-2 rounded-md py-1',
			$page.url.pathname === '/saved-cards' ? 'bg-muted' : 'text-muted-foreground'
		)}
		on:click={onItemClicked}
	>
	...
	<a
		...
		class={cn(
			'mx-[-0.65rem] flex items-center  px-3 hover:text-foreground',
			isSheetMode ? 'gap-4 rounded-xl py-2 text-lg font-medium' : 'gap-2 rounded-md py-1',
			$page.url.pathname === '/how-to' ? 'bg-muted' : 'text-muted-foreground'
		)}
		on:click={onItemClicked}
	>
